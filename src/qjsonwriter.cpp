...
static void valueToJson(const QJsonPrivate::Base *b, const QJsonPrivate::Value &v, QByteArray &json, int indent, bool compact)
{
    ...
    case QJsonValue::Double:
        {
            double d;
            if (std::modf(v.toDouble(b), &d) == 0) {
                if (d >= LONG_MIN && d <= ULONG_MAX) {
                    if (d < 0) {
                        json += QByteArray::number( (qint32) d);
                    } else {
                        json += QByteArray::number( (quint32) d);
                    }
                } else if (d >= LLONG_MIN && d <= ULLONG_MAX) {
                    if (d < 0) {
                        json += QByteArray::number( (qint64) d);
                    } else {
                        json += QByteArray::number( (quint64) d);
                    }
                } else {
